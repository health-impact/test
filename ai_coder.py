...
# 4. البرومبت المطور والمطهر تماماً مع ميزة الأخبار المتجددة بتاريخ اليوم
prompt = f"""
...
3. أمر إجباري ومقدس للصور: يُمنع منعاً باتاً تنزيل أو عرض أي خبر بدون صورة، ويُمنع ترك حقل الصورة فارغاً. يجب تزويد كل خبر برابط صورة عالية الجودة ومباشرة من Unsplash تعبر بدقة عن تفاصيل ومحتوى الخبر (كأنها الصورة الرسمية المنشورة مع الخبر في مصدره الموثوق مثل: أجهزة المختبرات، أطباء وفنيين، جراثيم تحت المجهر، أو أغذية سليمة).
...
رابعاً: مصفوفة الأخبار الحية المحدثة تلقائياً:
تأكد من أن الكود يحتوي في قسم الـ Script على مصفوفة باسم:
const healthNewsData = []; 
أمر إجباري: قم بملء هذه المصفوفة بـ 3 أخبار طبية وصحية حقيقية وبارزة جداً ومطابقة لتاريخ اليوم الحالي وهو ({current_date_str}). ركز على مجالات ومستجدات: مقاومة المضادات الحيوية، سلامة الأغذية، ومكافحة العدوى، مع روابط صورها الدلالية المباشرة والشغالة من Unsplash (بدون حقل التاريخ).
...
print(f"جاري تشغيل السكربت المطور لتحديث قسم الأخبار تلقائياً بتاريخ اليوم {current_date_str}... ⏳")

# 5. تنفيذ الطلب وحفظ الملف
try:
    ...
    print("تم التحديث بنجاح تام! 🚀 الأخبار أصبحت حية ومطابقة لتاريخ اليوم وصورها دلالية جاهزة.")
